...
namespace Transform2D
{
	...
	//with Radius
	inline glm::mat3 RotateAndRadius(float radians, float radius)
	{
		// TODO implement rotate matrix
		return  glm::transpose(glm::mat3(radius * cos(radians), radius  * -sin(radians), 0,
			radius * sin(radians), radius * cos(radians), 0,
			0, 0, radius));
	}
}
